...
def transact(account_id: str, payload: TransactionPayload, current_user=Depends(get_current_user)):
    return account_service.deposit_fund (account_id, payload.amount)


@account_router.post("/{account_id}/withdraw")
def withdraw(account_id: str, payload: TransactionPayload, current_user=Depends(get_current_user)):
    return account_service.withdraw_fund(account_id, payload.amount)